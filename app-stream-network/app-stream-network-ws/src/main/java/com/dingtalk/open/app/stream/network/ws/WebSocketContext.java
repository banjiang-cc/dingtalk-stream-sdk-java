...
public class WebSocketContext implements Context {
  ...
  public WebSocketContext(String connectionId, ProtocolRequestFacade request,
  ...
  public void replay(Object payload) {
    ...
    context.writeAndFlush(response).addListener(future -> {
      if (!future.isSuccess()) {
        LOGGER.error(
            "[DingTalk] websocket connection reply response failed, connectionId={}, messageId={}",
            connectionId, request.getMessageId(), future.cause());
        context.close();
      }
    });
  }
  ...
  public void exception(Throwable t) {
    ProtocolResponse response = t instanceof ServiceException ?
        ProtocolResponse.newErrorResponse(request, ((ServiceException) t).getCode(),
            t.getMessage()) : ProtocolResponse.new500Response(request);
    context.writeAndFlush(response).addListener(future -> {
      if (!future.isSuccess()) {
        LOGGER.error(
            "[DingTalk] websocket connection reply response failed, connectionId={}, messageId={}",
